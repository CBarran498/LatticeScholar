...
function analysisHtml(result) {
  ...
  const questions = (result.key_questions || []).map((item,index)=>{
    ...
    return `<article class="question-card answer-card"><div class="question-no">0${index+1}</div><div class="question-body"><h4>${escapeHtml(item.question)}</h4><div class="direct-answer"><span>结论</span><p>${escapeHtml(item.answer)}</p></div><ol class="answer-points">${pointList}</ol>${evidence}</div></article>`;
  }).join("");
  const warnings = (result.warnings || []).map(item=>`<li>${escapeHtml(item)}</li>`).join("");
  return `<div class="answer-head"><h3>论文四问</h3><div><span>${escapeHtml(modeLabels[result.mode] || result.mode)}</span><span>${escapeHtml(confidenceLabels[result.confidence] || result.confidence)}</span>${result.document ? `<span class="quality-${escapeHtml(result.document.quality)}">${escapeHtml(qualityLabels[result.document.quality] || result.document.quality)}</span>` : ""}</div></div>
