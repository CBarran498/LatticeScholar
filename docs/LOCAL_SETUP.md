...
**默认模式为 `accounts`（邮箱登录）**——首次打开会看到邮箱登录页。输入你的邮箱后，6 位验证码会发送到你的邮箱中，输入验证码即可进入工作台。验证码通过项目内置的邮件服务发送，无需配置 SMTP。建议在 `.env` 中设置 `LATTICE_ADMIN_EMAILS=your@email.edu` 以获得管理员（Pro）权益。不需要配置任何模型，所有本地能力开箱即用。
