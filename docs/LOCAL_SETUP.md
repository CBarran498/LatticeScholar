...
**默认模式为 `accounts`（邮箱登录）**——首次打开会看到邮箱登录页。输入邮箱后验证码直接显示在页面上（本地未配置 SMTP 时自动启用），输入验证码即可进入工作台。所有功能对登录用户完全免费开放。建议在 `.env` 中设置 `LATTICE_ADMIN_EMAILS=your@email.edu` 以获得管理员权限。不需要配置任何模型，所有本地能力开箱即用。
